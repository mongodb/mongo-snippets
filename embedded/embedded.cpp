...
int initEmbeddedMongo(int argc, char* argv[])
{
...

    {
		...
		// DEFAULT TO NOPREALLOC FOR EMBEDDED (hmmm wonder what is best...)
		...
		// DEFAULT TO SMALL FILES FOR EMBEDDED
	...
	/* TODO
	   we likely don't want to listen on a port at all for embedded.  however
	   it could be quite useful for development so it is on so far.
	*/
	boost::thread t(embeddedServerMainThread);
    return 0;
}
